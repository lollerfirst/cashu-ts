...
import {
	KvacBootstrapResponse,
	KvacCheckStateResponse,
	KvacMeltResponse,
	KvacMintResponse,
	KvacRestoreResponse,
	KvacSwapResponse
} from './model/types/mint/kvac/responses';
import {
	KvacBootstrapPayload,
	KvacCheckStatePayload,
	KvacMeltPayload,
	KvacMintPayload,
	KvacRestorePayload,
	KvacSwapPayload
...
export class ExtendedCashuMint extends CashuMint {
	...
	public static async kvacMelt(
		...
		if (!isObj(data) || typeof data?.state !== 'string' || !Array.isArray(data?.issued_macs)) {
			throw new Error('bad response');
		}
	...
	async kvacRestore(restorePayload: { tags: Array<Scalar> }): Promise<KvacRestoreResponse> {
	...
	public static async getKvacKeys(
		...
		const data = await requestInstance<MintActiveKvacKeys>({
			endpoint: keysetId
				? joinUrls(mintUrl, '/v2/kvac/keys', keysetId)
				: joinUrls(mintUrl, '/v2/kvac/keys')
		});
